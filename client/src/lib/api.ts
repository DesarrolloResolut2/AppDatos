...
export async function fetchCensoData(): Promise<CensoDataItem[]> {
  try {
    ...
    const [generalResponse, caceresResponse, badajozResponse, soriaResponse, teruelResponse, huescaResponse, lleidaResponse, jaenResponse, asturiasResponse, guadalajaraResponse] = await Promise.all([
      ...
      axios.get<INEDataItem[]>(CENSO_API_URLS.guadalajara)
    ]);
    ...
    // Process Jaén data
    const jaenData = jaenResponse.data.map(item => {
      const nombrePartes = item.Nombre.split(". ");
      let tipo: 'provincia' | 'municipio' = "municipio";
      let nombreLimpio = nombrePartes[0];

      // Si el nombre es 'Jaén' y contiene 'Total habitantes', es un dato provincial
      if (nombreLimpio === 'Jaén' && item.Nombre.includes('Total habitantes')) {
        tipo = 'provincia';
      }
    ...
    // Process Guadalajara data
    const guadalajaraData = guadalajaraResponse.data.map(item => {
      const nombrePartes = item.Nombre.split(". ");
      let tipo: 'provincia' | 'municipio' = "municipio";
      let nombreLimpio = nombrePartes[0];

      // Si el nombre es 'Guadalajara' y contiene 'Total habitantes', es un dato provincial
      if (nombreLimpio === 'Guadalajara' && item.Nombre.includes('Total habitantes')) {
        tipo = 'provincia';
      }
      ...
      return {
        ...
        nombreLimpio,
        genero,
        provincia: 'Guadalajara' as const
      };
    });

    return [...generalData, ...caceresData, ...badajozData, ...soriaData, ...teruelData, ...huescaData, ...lleidaData, ...jaenData, ...asturiasData, ...guadalajaraData];
  } catch (error) {
